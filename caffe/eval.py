...
'''
Eval results using pretrainned weights the given images using caffe googlenet architecture.
...
def load_args():
	...
	ap.add_argument('-d', '--dataset-path',
					...
					type=str, required=True)
	ap.add_argument('-sf', '--subset-fold',
					dest='subset_fold',
					help='inform if we should evaluate test, validation or train.',
					type=str, required=False, default='test')
	ap.add_argument('-tt', '--task-type',
					dest='task_type',
					help='classification or localization.',
					type=str, required=False, default='localization')
	ap.add_argument('-slf', '--dont-save-label-file', action='store_true',
					dest='dont_save_label_file',
					help='should save label files', 
					default=False, required=False)
...
if not args.imgs_list or not os.path.exists(args.imgs_list):
	if not args.dataset_path or not args.fold_id:
		...
		if (args.subset_fold == 'train'):
			list_positive_file      = os.path.join(os.path.dirname(args.dataset_path), 'folds', args.fold_id, 'positive_svm_training_set.txt')
			list_negative_file      = os.path.join(os.path.dirname(args.dataset_path), 'folds', args.fold_id, 'negative_svm_training_set.txt')
		elif(args.subset_fold == 'validation'):
			list_positive_file = os.path.join(os.path.dirname(args.dataset_path), 'folds', args.fold_id, 'positive_svm_validation_set.txt')
			list_negative_file = os.path.join(os.path.dirname(args.dataset_path), 'folds', args.fold_id, 'negative_svm_validation_set.txt')
		elif(args.subset_fold == 'test'):
			list_positive_file       = os.path.join(os.path.dirname(args.dataset_path), 'folds', args.fold_id + '_positive_test.txt')
			list_negative_file       = os.path.join(os.path.dirname(args.dataset_path), 'folds', args.fold_id + '_negative_test.txt')

		full_list = get_file_list_content(list_positive_file) + get_file_list_content(list_negative_file)
		
		full_list.sort()
		overall_input = mount_fold_list(args.input_dir, full_list)
...
for bidx, fidx in enumerate(xrange(0, nfiles, args.minibatch_size)):
	...
	for line in out['prob']:
		if i >= len(toprocess_input):
			print  >> sys.stderr, "An Error has ocurred while processing the images"
			break

		#toprocess_input[i] is like /data/formatted_2/1_s/frames/vNonPorn000016/vNonPorn000016.0000001.1.0.tif
		#we will get the relative piece vNonPorn000016/vNonPorn000016.0000001.1.0.tif
		rel_fname = os.path.relpath(toprocess_input[i], args.input_dir)
		#and join to output dir
		output_fname_label = os.path.join(args.output_dir, rel_fname + ".label")
		#So we collect just the dir name to create the dir, if needed
		output_video_dir = os.path.dirname(output_fname_label)
		#we save if the flag to not save label files is not true
		if not args.dont_save_label_file:
		...
		if(args.task_type == 'classification'):
			if('vNonPorn' in rel_fname):
				cpu_label = '0'
			else:
				cpu_label = '1'
		else: #localization
			cpu_label = rel_fname.split('.')[3]
		#print 'rel_fname %s, line_label %s, cpu_label %s, ' % (rel_fname, line_label, cpu_label)
		if(cpu_label == '1'):
			ClassPorn = ClassPorn + 1
			if(line[0] < line[1] ):
				ClassPornHit = ClassPornHit + 1
		else:
			ClassNonPorn = ClassNonPorn + 1
			if(line[0] > line[1] ):
				ClassNonPornHit = ClassNonPornHit + 1
	...
	if(ClassPorn == 0):
		...
		acc_porn = ClassPornHit/float(ClassPorn)
	
	if(ClassNonPorn == 0):
		acc_nonporn = 0
	else:
		acc_nonporn = ClassNonPornHit/float(ClassNonPorn)

	acc = (ClassPornHit + ClassNonPornHit)/float(ClassPorn + ClassNonPorn)
	bacc = (acc_porn + acc_nonporn)/2
	print 'minibatch %d out of %d took %g seconds to process %d images, acc %.2f, bacc %.2f, acc_nonporn [%d/%d] %.2f, acc_porn [%d/%d] %.2f' % (
		bidx+1, nbatches, time.time()-t0, len(toprocess_input), acc*100, bacc*100, ClassNonPornHit, ClassNonPorn, acc_nonporn, ClassPornHit, ClassPorn, acc_porn)
	
f = open(os.path.join(args.output_dir, 'results_'+ args.task_type + '_' + args.subset_fold + '.txt'), "w")
f.write('Final Result acc %.2f, bacc %.2f, acc_nonporn [%d/%d] %.2f, acc_porn [%d/%d] %.2f' % (
		acc*100, bacc*100, ClassNonPornHit, ClassNonPorn, acc_nonporn*100, ClassPornHit, ClassPorn, acc_porn*100))
f.close()
