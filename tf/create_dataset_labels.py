...
def create_labels_by_fold(input_dir, output_dir_path, train_type, fold_id, dataset_path):
	...
	svm_train_list_positive_file = os.path.join(os.path.dirname(dataset_path), 'folds', fold_id, 'positive_svm_training_set.txt')
	svm_train_list_negative_file = os.path.join(os.path.dirname(dataset_path), 'folds', fold_id, 'negative_svm_training_set.txt')

	svm_validation_list_positive_file = os.path.join(os.path.dirname(dataset_path), 'folds', fold_id, 'positive_svm_validation_set.txt')
	svm_validation_list_negative_file = os.path.join(os.path.dirname(dataset_path), 'folds', fold_id, 'negative_svm_validation_set.txt')
...
#	print "test_list_negative_file : %s" % test_list_negative_file
	...
	svm_train_list      = get_file_list_content(svm_train_list_positive_file) + get_file_list_content(svm_train_list_negative_file)
	svm_validation_list = get_file_list_content(svm_validation_list_positive_file) + get_file_list_content(svm_validation_list_negative_file)

	random.shuffle(train_list)
	random.shuffle(validation_list)
	random.shuffle(test_list)

	random.shuffle(svm_train_list)
	random.shuffle(svm_validation_list)
...
#	print "test_list : %s" % test_list
	...
	svm_train_list_label_file = os.path.dirname(output_dir_path) + "/" + fold_id + "_svm_train_list.label"
	svm_validation_list_label_file = os.path.dirname(output_dir_path) + "/" + fold_id + "_svm_validation_list.label"
	...
	if os.path.isfile(svm_train_list_label_file):
		print "Will remove %s" % svm_train_list_label_file
		os.remove(svm_train_list_label_file)
	if os.path.isfile(svm_validation_list_label_file):
		print "Will remove %s" % svm_validation_list_label_file
		os.remove(svm_validation_list_label_file)
	...
	for video in svm_train_list:
		mount_labels(input_dir, video, train_type, svm_train_list_label_file)

	for video in svm_validation_list:
		mount_labels(input_dir, video, train_type, svm_validation_list_label_file)
